// src/pages/Admin.jsx — v4: featured toggle, multi-image, improved UI
import { useState, useEffect, useRef } from 'react';
import { supabase, ADMIN_EMAIL, uploadCoverImage } from '../supabase';
import { useBooks } from '../hooks/useBooks';
import './Admin.css';

const CATEGORIES = ['Islamic','Fiction','Non-Fiction','History','Science','Philosophy','Poetry','Children','Other'];

const EMPTY_FORM = {
  title: '', author: '', price: '', description: '',
  category: '', imageUrl: '', imageFile: null,
  is_featured: false,   // Fix 8
  extraImages: [],      // Fix 7: extra image URLs
};
...
export default function Admin() {
  ...
  const handleLogin = async (e) => {
    ...
    if (authError) {
      ...
      setLoginErr(c.includes('invalid') || c.includes('credentials') || c.includes('password')
  ...
  const openAdd = () => {
    setEditingId(null); setForm(EMPTY_FORM); setFormErrors({}); setPreviewUrl(''); setShowModal(true);
  };
  ...
  const openEdit = (book) => {
    ...
    setForm({
      ...
      extraImages: Array.isArray(book.images) ? book.images.filter(u => u !== book.image_url) : [],
    });
    setFormErrors({});
  ...
  const closeModal = () => {
    if (saving) return;
    setShowModal(false); setEditingId(null); setForm(EMPTY_FORM); setFormErrors({}); setUploadProgress(null);
  };
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      // Fix 7: build images array = [primary, ...extras]
      const allImages = [imageUrl, ...form.extraImages].filter(Boolean);
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try { await deleteBook(deleteTarget.id); success('Book deleted.'); }
      ...
      <div className="admin-login__card scale-in">
        ...
        <form className="admin-login__form" onSubmit={handleLogin} noValidate>
          ...
          <div className="admin-login__field">
            ...
            <div className="admin-login__input-wrap">
              <svg className="admin-login__input-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
  ...
  return (
    <div className="admin">
      <aside className="admin-sidebar">
        ...
        <div className="admin-sidebar__footer">
          <img
            ...
            className="admin-sidebar__avatar" alt="avatar"
          />
      ...
      <main className="admin-main">
        ...
        {/* Fix 6: Improved stat cards */}
        <div className="admin-stats">
          {[
            { label:'Total Books', value: books.length, icon:'📚', color:'#7c4dbd' },
            { label:'Featured',    value: books.filter(b=>b.is_featured).length, icon:'⭐', color:'#d4a847' },
            { label:'With Price',  value: books.filter(b=>b.price).length,  icon:'💰', color:'#16a34a' },
            { label:'Free Books',  value: books.filter(b=>!b.price).length, icon:'🎁', color:'#0369a1' },
          ].map(s => (
            <div key={s.label} className="admin-stat-card" style={{ '--stat-color': s.color }}>
              <div className="admin-stat-card__icon-wrap"><span className="admin-stat-card__icon">{s.icon}</span></div>
        ...
        <div className="admin-table-wrap">
          {booksLoading ? (
            ...
            <div className="admin-empty"><span>{search || filterCat !== 'All' ? '🔍 No matching books.' : '📭 No books yet. Add your first!'}</span></div>
          ) : (
            <table className="admin-table">
              <thead>
                <tr>
                  <th>Cover</th><th>Title</th><th>Category</th><th>Price</th>
                  {/* Fix 8: featured column */}
                  ...
                  <tr key={book.id} className="admin-table__row" style={{animationDelay:`${i*30}ms`}}>
                    <td>
                      <img className="admin-table__cover"
                        src={book.image_url || 'https://placehold.co/56x76/1a1a2e/b48de8?text=📖'}
                        alt={book.title}
                        onError={e => { e.target.src='https://placehold.co/56x76/1a1a2e/b48de8?text=📖'; }}
                      />
                    </td>
                    <td>
                      <div className="admin-table__title">{book.title}</div>
                      {book.author && <div className="admin-table__author">{book.author}</div>}
                      {/* Fix 7: show image count */}
                      {Array.isArray(book.images) && book.images.length > 1 && (
                        <div className="admin-table__img-count">🖼 {book.images.length} images</div>
                      )}
                    </td>
                    ...
                    <td style={{textAlign:'center'}}>
                      <button
                        className={`admin-featured-toggle ${book.is_featured ? 'active' : ''}`}
                        onClick={async () => {
                          try {
                            await updateBook(book.id, { ...book, imageUrl: book.image_url, images: book.images || [], is_featured: !book.is_featured });
              ...
              <div className="admin-modal__layout">
                <div className="admin-modal__fields">
                  <FormField label="Title *" error={formErrors.title}>
                    <input value={form.title} onChange={e => setField('title', e.target.value)} placeholder="e.g. The Alchemist" className={formErrors.title ? 'error' : ''} />
                  </FormField>
                  ...
                  <div className="admin-modal__row">
                    ...
                    <FormField label="Price (MAD)" error={formErrors.price}>
                      <input type="number" min="0" step="0.01" value={form.price} onChange={e => setField('price', e.target.value)} placeholder="0.00" className={formErrors.price ? 'error' : ''} />
                    </FormField>
                  </div>
                  <FormField label="Description">
                    <textarea value={form.description} onChange={e => setField('description', e.target.value)} placeholder="A short description…" rows={3} />
                  </FormField>

                  {/* Fix 8: Featured toggle */}
                  <div className="admin-featured-field">
                    <label className="admin-featured-label">
                      <input
                        type="checkbox"
                        checked={form.is_featured}
                        onChange={e => setField('is_featured', e.target.checked)}
                        className="admin-featured-checkbox"
                      />
                      <span className="admin-featured-switch" />
                      <span>⭐ Featured Product</span>
                      <span className="admin-featured-hint">Shown on home page</span>
                    </label>
                  </div>

                  <FormField label="Primary Image URL" error={formErrors.imageUrl}>
                  ...
                  <div className="admin-modal__or"><span>or upload</span></div>
                  <div className="admin-upload-zone"
                    onClick={() => fileInputRef.current?.click()}
                    onDragOver={e => e.preventDefault()}
                    onDrop={e => { e.preventDefault(); const f = e.dataTransfer.files[0]; if (f?.type.startsWith('image/')) { setField('imageFile', f); setField('imageUrl', ''); } }}
                  >
                    <input ref={fileInputRef} type="file" accept="image/*" style={{display:'none'}}
                      onChange={e => { const f = e.target.files[0]; if (f) { setField('imageFile', f); setField('imageUrl', ''); } }} />
                    ...
                    <span>{form.imageFile ? form.imageFile.name : 'Click or drag & drop'}</span>
                    <span className="admin-upload-zone__hint">PNG, JPG, WEBP · max 5 MB</span>
                  </div>
                  {uploadProgress !== null && (
                  ...
                  <FormField label="Additional Image URLs">
                    {form.extraImages.map((url, idx) => (
                      <div key={idx} className="admin-extra-img-row">
                        <input
                          value={url}
                          ...
                          onChange={e => {
                            ...
                            setField('extraImages', next);
                          }}
                        />
                        <button type="button" className="admin-extra-img-remove"
                          onClick={() => setField('extraImages', form.extraImages.filter((_, i) => i !== idx))}>✕</button>
                      </div>
                    ))}
                    <button type="button" className="admin-extra-img-add"
                      onClick={() => setField('extraImages', [...form.extraImages, ''])}>
                      + Add Image URL
                    </button>
                  </FormField>
                </div>
                ...
                <div className="admin-modal__preview-col">
                  ...
                  {form.is_featured && (
                    <div className="admin-modal__featured-badge">⭐ Featured</div>
                  )}
                </div>
              </div>
